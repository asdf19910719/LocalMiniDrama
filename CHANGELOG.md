...
- **V2.1 常规设置三处"展示与实现不符"修复**：默认画幅保存此前仅改前端状态、只落语言不落画幅（假保存），现随创作默认值端点真实落库；备份区「立即创建备份」由 comingSoon 占位改为真实备份执行；媒体/成片/临时目录行由恒显示"正常"的静态徽标改为「未检测」默认态，状态经「重新检测」真实获取后呈现（异常时展示后端返回的具体原因）
