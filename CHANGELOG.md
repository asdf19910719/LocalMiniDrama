...
- **V2.1 项目素材删除确认先行（B7）**：项目素材详情抽屉的「删除」不再先删除后确认——此前点删除会立即把素材移入回收站，用户在随后弹出的确认框点"取消"时素材已被删除，确认后还会二次调用删除接口报 404；现改为先弹「移入回收站」自建确认弹窗（含可恢复删除说明），确认后才调用一次删除接口，成功后关闭抽屉、刷新列表并提示"已移入回收站"；素材被分镜引用阻塞或删除失败时以警示条提示且素材保留，页面顶部新增可关闭的提示条承接上述反馈

### 新增

- **V2.1 项目素材生成前确认 Sheet（P0-7 最小闭环）**：项目素材详情抽屉的两个生成入口（候选行"+"与页脚「生成候选」）不再静默直接生成——统一先打开生成确认 Sheet，按固定顺序展示生成对象（类型 + 名称）、生成通道（本地生成 · mock 通道）、画布尺寸（按素材类型默认：人物 720x960 · 场景 1280x720 · 道具 720x720）与可编辑提示词（默认取素材名称）、任务与费用摘要（1 个生成任务 · 本地生成不产生 API 费用），确认后才调用生成接口并提交 Sheet 内编辑后的提示词；成功提示"已生成候选"并刷新详情候选列表，失败以警示条提示，提交中按钮禁用防重复提交
