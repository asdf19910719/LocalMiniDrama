...
- **V2.1 分镜页三个确认/预览抽屉与 `?shot=` 镜头定位（付费安全与失效传播可见性）**：分镜页三处高影响操作补确认前置——「生成/重新生成 H3 提示词」先经确认抽屉展示结构输入（时段数、引用槽位数）与来源说明「基于当前时段与引用状态」，草稿被人工编辑过时须勾选「我确认覆盖人工修改」才能提交（透传后端 `confirmOverwrite`，不再撞 H3_MANUAL_PROTECTED 静默失败），失败信息就地呈现在抽屉内；分镜图候选缩略图由点击即采纳改为打开预览抽屉（大图 + 本镜分镜图提示词 + 生成时间），点「设为当前分镜图」确认后才写入当前图；「进入成片审核」先弹摘要抽屉展示三组计数（候选未采用·需确认 / 尚未生成 / 生成失败，取自批量预检与完成度统计），可「留在分镜」或「仍要进入成片审核」。另：成片页「回分镜处理」携带的 `?shot=` 参数现被分镜页消费——加载后自动选中该镜头并清除查询参数；「生成历史」改经 openHistory 先清空陈旧数据再拉取后开门，切镜时历史抽屉开着会自动重拉，不再展示上一镜的任务；v21-ui.css 补齐 `.notice-strip.danger` 变体，成片页生成失败提示条不再是默认灰底
