...
- **V2.1 剧本浏览器恢复副本（丢稿防护）**：剧本编辑页新增本机自动快照——每次编辑后 600ms 节流把当前正文写入浏览器 localStorage（按剧集隔离），保存成功后自动清除；进入剧本页时若检测到与服务端草稿不一致的本地副本，右侧「修订状态」卡显示警示卡与「查看」入口，打开「本地副本」对比弹窗（本地副本 vs 当前草稿双栏），提供「恢复本地副本 / 丢弃本地副本 / 保留（稍后处理）」三动作——恢复后标记为未保存修改由用户确认保存，不做静默写库。覆盖自动保存失败、浏览器崩溃、误关页导致的本机丢稿场景。
- **V2.1 项目素材回收站筛选与恢复入口**：项目素材页工具栏新增「回收站」段——切入后列表改走回收站口径（后端 `GET /api/v2/projects/:id/assets` 新增 `recycled=true` 参数，返回软删素材并带 `deletedAt`；恢复走既有 `POST /api/v2/assets/:type/:assetId/restore` 端点，前端封装 `restoreAsset` 零调用清零），回收站卡片显示移入时间并提供「恢复」按钮（成功 toast 并刷新，失败就地提示）；统计卡与新增入口在回收站视图下隐藏，空态区分「回收站为空」。素材删除确认文案由「可在高级数据工具中恢复删除」改为「可在本页回收站筛选中恢复」，承诺与实际入口一致。
- **V2.1 归档项目恢复入口**：项目列表「已归档」页签的卡片操作菜单由「移入回收站」换为「恢复项目」——调用既有 `POST /api/v2/projects/:id/restore` 端点（前端封装 `restoreProject` 零调用清零），成功 toast 提示并刷新列表，失败原因 toast 呈现，恢复中按钮防重复；非归档页签行为不变。
- **V2.1 风格版本记录真实化**：「查看风格」抽屉的「版本记录」区由「版本历史暂未记录」占位改为渲染真实应用历史——后端新增只读端点 `GET /api/v2/projects/:id/style/versions`（读取既有 `project_style_events` 审计表，倒序限 50，每次「确认更换风格」均已留痕），抽屉逐条显示「第 N 次 · 应用风格 · 时间」；无更换记录时据实显示「本项目还没有更换风格的记录」。
- **V2.1 剧集来源标签细分（制作包 / 小说拆集 / 外部 AI）**：剧集中心来源标签不再把所有导入混为一谈——后端剧集列表新增 `importSchema` 字段（取该集最近一条导入记录的协议名），小说拆集确认时为每个创建集补写 `episode_imports` 来源记录（`schema_name='novel-split'`，协议/时间随「查看导入来源」抽屉可读）；前端标签按协议分流显示「制作包导入 / 小说拆集 / 外部 AI 导入」（此前小说拆集与制作包创建的集被误标为「手工创建」），导入来源抽屉协议行同步改用用户语言并保留原始协议名。
- **V2.1 归档导入真实校验（`POST /api/v2/archive/validate`，Task 5-C）**：导入项目归档页的「校验并确认」从三条静态检查占位升级为真实校验。后端新端点收 JSON `{ path }`（本地 zip 路径，不引入上传基建；导入动作本身保持既有 `POST /api/v1/dramas/import` 文件上传），用 adm-zip 读取归档并返回七项检查矩阵——格式 / 协议版本 / 结构 / 解压完整 / 媒体条目 / 项目重名 / 磁盘空间，逐项「通过 / 警告 / 阻断 / 版本不支持」并附原因，另附概要指标（项目名 / 剧集数 / 媒体文件数 / 条目数 / 预计解压大小 / zip 大小）；硬错误按统一口径返回：路径缺失 400，文件不存在 / 路径非文件 / 非 zip 一律 400 + 同一模糊文案「无法读取该归档文件」（不区分具体失败原因，消除存在性探测差分）。版本校验对照 2.1 协议：现有 V1 导出（project.json version=1.7）如实返回 `unsupported` 与实际版本号，不伪装通过；磁盘空间检查探测数据目录所在盘并在结果中注明「数据盘」（导入的媒体落在数据盘）。页面侧（归档导入页）：新增归档本地路径输入（校验失败保留输入可就地重试），三条静态检查行替换为后端检查矩阵渲染，概要区显示指标；unsupported 时呈现"检测到归档版本 vX，当前仅支持 2.1"横幅与「重新选择」；存在阻断项（overall error）时导入按钮禁用并提示「存在阻断项，无法导入」，校验后再改动路径会提示"路径已变更"并要求重新校验（校验与导入指向同一文件）；页面注明「导入不覆盖现有项目」，仅填路径未选文件时导入按钮禁用并说明导入走文件上传。后端配套 `backend-node/src/v21/import/archiveValidateService.js`，新增 4 条测试（缺 path 400 / 不存在与非 zip 同文案 400 / 真实导出全矩阵与重名 warn + 数据盘口径 / 缺 manifest 结构阻断）
...
- **V2.1 更换画面风格对齐旧版选择器交互（预览图 / 一步更换 / 详情 / 创建我的风格）**：项目概览「更换风格」弹窗四项修复——①风格卡片由渐变占位块改为渲染真实预览图（169 张风格预览图本就在 `frontweb/public/style-thumbs/` 且接口已返回，此前未接线），弹窗加宽至 1120px、预览图按 16:10 比例展示（约 257×160，对齐旧版尺寸，替代原 96px 高小图），无图回退占位渐变；②交互由「选择 → 确认影响 → 应用」两步简化为「点卡片选中 → 右下角确认更换」一步（影响口径保留在选择页底部说明），选中卡片加 2px 主题色描边与「已选」徽标——此前选中态仅细边框几乎不可见，被误认为点击无反应；③卡片新增「详情」视图：大图预览、名称/说明/分类与中英文风格提示词**默认展开**显示全文（对齐旧版风格选择器的详情抽屉，可点击收起）；④恢复「创建我的风格」入口（中英文名称/说明/中英文提示词五字段必填，走既有 `POST /api/v1/styles/custom` 端点落 `custom_styles` 表，创建后自动切到「我的风格」页签并选中），并删除「需安装自定义风格目录后开放」的错误禁用文案——该创建能力后端一直默认支持，无需任何目录安装。
- **V2.1 制作头运行任务徽标接真实计数与费用摘要**：制作头「任务」按钮旁的运行任务徽标由恒为 0 的死值改为接任务聚合端点真实进行中计数（`GET /api/v2/tasks?status=in_progress` 的 total，30 秒轮询保持新鲜，失败保留上次计数、为 0 时不渲染徽标不伪造数字）；按钮左侧新增「费用 · 本地 ¥0」摘要 chip，悬停说明"当前生成为本地通道执行，不产生 API 费用；接入外部付费通道后此处显示真实费用口径"。
- **V2.1 外部 AI 向导「自动汇总上下文」步接真实数据**：上下文步由静态占位文案改为展示可预知的真实信息——目标（创建第 N 集 / 填充第 N 集，集号由本地列表真实计算，同时修复「选择目标」步「创建第 N 集」集号空缺显示"第 集"的问题）、项目素材对象数（来自项目概览聚合，注明将冻结进素材快照）；已创建任务包的场景（离页恢复）回显任务真实上下文版本指纹、素材快照摘要指纹与生成时间；尚未创建任务包时如实标注各项数值"创建任务包时生成/冻结"，不伪造。
- **V2.1 新建项目来源流程接续**：新建项目选择「小说 / 长文本拆集」「从已有视频开始剪辑」来源后，创建完成不再回落到空的剧集列表，而是直达对应导入向导（小说拆集向导 / 已有视频登记向导），续上所选来源的创作流程。
- **V2.1 UI 剩余清单收口（第二轮逐屏美化）**：完成首轮核对报告列出的剩余项——AI 配置页解析顺序链首项「本次任务覆盖」高亮为强调色并把冻结说明移到链下方独立行、三通道卡补 38px 彩色图标块（API 紫 / ChatGPT 绿 / ComfyUI 蓝）；高级数据工具完整性检查结果改为三张大数字统计卡（21px 着色数字替代一行小徽标）、问题行拆两级（标题 + 副文本，恢复动作升级为按钮）、左侧工具导航激活态由实心紫主按钮改为 subtle 底 + 图标（对齐"当前选择 = subtle 底"规范）、物理清理页头补「不可恢复」danger 徽标与命中/阻断汇总、逐文件清单升级为五列表格（路径/大小/判定/勾选，阻断原因按语义分色：有引用 warn / 任务占用 info / 路径越界 danger，阻断行降透明度）、汇总行加粗可回收字节数；归档导入校验结果由纯文本行升级为双列检查卡网格（图标色块 + 标题 + 状态着色详情）；四个向导（制作包导入/归档导入/小说拆集/已有视频）的局部步骤条样式删除、收敛到全局三态节点（完成 = 绿勾图标，替代文本 ✓）；资产库缩略比例落地 3:4/16:9/1:1、徽标上移左上、删除无信息量"v1"版本徽标、详情抽屉重排为左图右资料双栏（头部补类型徽标与文件不可访问警示、文件状态行补"可访问/不可访问"状态着色、说明条升级 info 提示卡、"用于项目"在文件不可访问时禁用、归档按钮补描边）；分镜页引用管理抽屉行升级为卡片形态（@槽位紫色标注 + 版本号 + 场景"不可移除"徽标 + danger 图标移除钮 + info 提示卡 + Footer「完成」主按钮）、素材预览抽屉重排为 180px 3:4 大图 + 右侧资料双栏（头部类型徽标、补「版本信息」小节与换绑后果说明、动作移入 Footer 并以主按钮呈现、宽度对齐 520px 档位）、生成条补「预计耗时」chip 与真实通道价格副文本（报价随切镜刷新）；自由创作页 Recipe 卡改横排（图标居左 + 右箭头、hover 回归描边加深）、页头常驻超长 warn 徽标改为 outline 短徽标 + 页内 warn 提示条；常规设置保存控件由页头移至底部 sticky 保存条（长表单滚动到中部时保存态与保存按钮常驻可见，页头改范围说明 outline 徽标）、只读路径由输入框形态改为 code 代码形态、卡片列居中（860px）、数据工具四宫格 hover 回归描边加深、工作区迁移向导步骤条由 seg 分段控件改为三态节点、范围预览 tiles 化（含任务/备份四格）并在阻断态补 danger 盾牌提示卡、执行态补进度条与「迁移中」徽标；高级画布容器加点阵背景与同色箭头（生成线蓝色箭头）、过期标签加底色防连线穿插、图例改线样、页头开发备注文案改用户语言；Rail 图标 tooltip 由原生 title 改为自绘（无延迟、随主题、键盘焦点可见）；新建项目页保存位置行补「更改」按钮（直达设置）；项目概览「下一步」卡补阶段进度条（解析 stageMeta 计数）与集名展示、项目操作菜单补外点关闭；项目素材页项目内导航去容器化；剧集中心列表底部补回收站 30 天可恢复提示行
- **V2.1 分镜页镜头下拉恒空白修复（剩余清单排查项）**：分镜页工具栏的「镜头选择」下拉此前绑定不存在的 `currentShotIdProxy`（data/computed 均未定义），选择器永远显示空白；现补读写计算属性桥接到镜头选中逻辑，下拉正确显示当前镜头并可选择切换
- **V2.1 制作工作台制作头增强（§4.3 最小闭环，Task 5-C）**：制作头「第 N 集 · 标题」改为剧集下拉（`v21.listEpisodes` 同项目全部剧集），选择即跳该集同阶段；剧本阶段有未保存修改时切换先出守卫——「保存并切换」（调用剧本页经 studioSave 通道注入的保存方法，保存失败留在本集可重试）或「放弃并切换」二选一，不再静默丢稿；其余阶段无该通道视为非 dirty 直接切。阶段组件以 episodeId 为 key 随切集强制重建（同路由记录内切集不再复用旧集实例——头部/导航/阶段内容同步刷新，且剧本页 800ms 自动保存定时器在卸载前清除，被放弃或未保存的草稿不会在切集后仍写回库）；阶段导航 warn 徽标补 title 提示（拼入阶段 meta 说明）；保存态透传真实语义：非剧本阶段文案改为「更改实时生效」（成片/设定页无草稿保存概念），剧本阶段保持既有注入通道（现同步上报 dirty 标记与保存方法，卸载时清理防残留误触发守卫）
- **V2.1 P2 批量清理七项（Task 5-C）**：剧集中心已完成（completed）剧集主按钮由「继续制作」改为「查看成片」直达该集成片阶段，搜索输入加 280ms 防抖（对齐项目列表，停止输入后才请求）；分镜页镜头轨筛选补「失败 / 处理中」两项并修复筛选真实生效（此前筛选段选不参与镜头过滤——后端 `batchPrecheck` 新增 `processing` 返回有进行中视频任务的镜头，前端加载时拉取预检作为失败/处理中计数与过滤来源）；分镜候选条与播放器对基于旧分镜图的视频候选加「基于旧分镜图 / 旧图」小标（消费换图接口返回的 `staleVideos` 候选 id 与 `getStoryboard` 完成度 `staleShots`）；分镜页 H3 主按钮文案「重新生成」补全为「重新生成 H3 提示词」；分镜空态补副文案「从已确认剧本创建镜头，或导入分镜结构」（分镜结构导入属后续后端工作，本期仅补文案）；高级数据工具迁移区徽标加载中显示「读取中…」（加载完成前不再提前显示「无记录」）
- **V2.1 页面状态机三分统一与 URL 可恢复核心 4 处（Phase 5 横切 B）**：多页"加载失败伪装成空态、无加载骨架、空态抢跑"系统性收口——新增极简复用组件 `frontweb/src/components/v21/StateBlock.vue`（props：`state`/`message`/`retry`/`icon`，error 态重试按钮 emit `retry` 由各页回调各自 load，empty 态提供动作插槽；行骨架样式入 `v21-ui.css`），项目列表、项目概览、剧集中心、资产库、成片页、任务中心、项目素材、分镜页统一接入「加载骨架 → 错误重试 → 内容」三分链，明确规则"加载完成前不渲染空态"（`loaded` 标志守卫）。逐页修复：项目列表 `load` 此前无 catch（失败伪装成"还没有项目"）、无加载骨架，现补齐并区分"筛选无结果（附清除条件）"与真空态；项目概览补加载骨架（加载中不再空数据渲染），错误卡收口为 StateBlock 统一呈现（重试语义不变）；剧集中心 `load` 此前无 catch，现补齐且空态带加载完成守卫；资产库 `load` 失败此前静默伪装空态、`loaded` 标志在请求发出前即置位（空态抢跑），现改为失败呈现可重试错误态、数据到达后才置位，并区分"筛选无结果（清除条件）"与"资产库为空"；本集设定页加载期间不再闪现"本集剧本没有引用…"空态文案（G6 审计项）；成片页补加载态与失败呈现（原为无兜底裸 await，失败页面白板，G8 审计项）；任务中心空态文案补加载完成守卫；高级数据工具各工具加载的 catch 与加载/空态区分经回归断言守卫（P4.4 已具备，跳过改动）。URL 可恢复核心 4 处（均 replace 写入、不产生历史记录）：项目素材 `?asset=<type>:<id>` 深链在列表加载后自动打开对应素材详情抽屉，打开详情时写入、关闭时仅清除该参数（保留列表筛选，规格 ASSETS-030 本期最小范围）；分镜页 `?scene=<id>` 场次筛选恢复并与既有 `?shot=` 一致持久化——切换场次/镜头时写回 URL、刷新可恢复选中状态（深链消费后不再单向清除，镜头不存在时给页内提示）；任务中心 `?tab=` 页签恢复（`?focus=` 深链既有行为保留，`in_progress` 默认页签不写入 URL 保持干净）；剧集中心 `?status=&q=` 筛选恢复，切换状态页签与搜索输入时同步写入（既有 `?stage=/imported/highlight` 行为不变）。前端 360 测试全绿（新增 11 条状态机与 URL 合同断言）、build 通过，后端零改动
...
- **V2.1 项目素材创建修复与生图提示词补全（回归修复 + 对齐旧版 v1 创建输入）**：①「创建素材后列表看不到」根因修复——上一批为素材列表加回收站口径时，HTTP query 透传的 `recycled=false` 是字符串（truthy），被后端误判为回收站视图，导致默认列表恒为空、新建素材"消失"；现 `recycled` 显式按 `'true'` 判定，字符串/布尔口径均安全，并补回归测试。②创建素材表单补「生图提示词」「负向提示词」两个可选输入（对齐旧版 v1 创建流程的 `prompt`/`negative_prompt` 字段：角色落 `polished_prompt`，场景/道具落 `prompt`），素材详情同步透出已保存提示词；生成候选的默认提示词改为「素材已保存提示词 → 素材名」回退链（生成确认 Sheet 与本集设定生成入口同步），不再是干巴巴的素材名。③创建请求补错误处理（失败时弹窗内呈现原因、按钮防重复，不再静默），创建成功后自动切换到所建类型筛选并 toast 提示，不再出现"创建了但列表看不到"的困惑。后端行为测试（创建带提示词三类素材、详情透出、生成回退链、recycled 字符串口径）6 条全绿；前端 386 测试全绿、构建通过，并经浏览器端到端实测（创建→列表可见→测试数据已清理）。
- **V2.1 外部 AI 结果导入根因修复：任务包格式与导入器协议对齐（storyboards/local_ref 全链贯通）**——外部 AI 按任务包《返回格式.schema.json》生成的结果此前必然导入失败：V2.1 外部 AI 向导的导入适配器（`externalAiWizardService.adaptResult`）实际期待的是内部规范格式（`story_scenes`/`shot_packages`/`source_key`），与任务包教会外部 AI 的格式（`storyboards`/`local_ref`/`prompt_contract`）是两套互不相识的协议，按包生成的结果先卡「素材快照摘要不一致」（Schema `additionalProperties:false` 禁止携带 assets_digest，导入端却硬性要求），走冻结快照通道也会因 `story_scenes`/`shot_packages` 缺失被拒——即"结果完全合规也导不进去"。已落地：①新增确定性转换器 `backend-node/src/v21/wizard/externalResultToV21.js`，把任务包格式结果转换为规范 episode-package@2.1——既有资产按冻结清单以 source_key 展开、新资产分配 `ai_<任务短码>_` 前缀 key、分镜按 scene_ref 首次出现聚合为场次、单分镜确定性生成时码闭合的时段（[0, duration_seconds]，对白按时长均分并解析说话人）、转场写入连戏 exit、声音设计写入 shot.audio；引用完整性校验（场景/人物/状态/道具归属）与向导校验清单共用同一规则（`collectReferenceErrors`），引用错误 400 可自诊。②`assets_digest` 回执缺失不再硬失败（按 Schema 本就可省略）：校验清单该项通过并注明"结果未携带"；提供但不一致时仍按原设计走「按冻结快照导入」面板。③Schema 与说明的自相矛盾清偿（上一条目遗留 P1）：运行时契约 `EXTERNAL_AI_RESULT_SCHEMA` 与 `basicResponseSchema` 均声明 `assets_digest`（可选字符串），发布版 `docs/单集制作包导入/制作包schema.json`、`制作包示例.json`、`上游AI生成提示词模板.md` 同步更新（合同测试守卫通过）。④导入引擎 `episodeImportV21` 补写分镜提示词列：canonical `base_video_prompt` 落 `video_prompt`，转换器保留在 `shot.extensions` 的万能提示词 `universal_segment_text`、`base_image_prompt`、画面描述、景别、声音设计 JSON、转场、对白/旁白文本、is_primary 落对应列（对既有无 extensions 的制作包零影响）——导入的草稿在分镜页/生成任务里提示词不再是空壳。前端向导零改动（校验清单与失配面板语义兼容）。测试：重写 `v21ExternalAiWizardTasks.test.js`/`v21ImportV21.test.js` 外部 AI 夹具为任务包格式（此前夹具用内部格式，掩盖了协议误配），新增包格式端到端导入、提示词落库、digest 缺失容错、引用错误不落库等回归；真实环境验证——后端重启后新建任务包（说明带 digest 回执、Schema 声明字段）→ 模拟外部 AI 结果 → 校验 7 项全过 → 预览 → 导入成功 → 分镜表 image_prompt/video_prompt/universal_segment_text/transition/is_primary 全部落库【用户操作提示：此前的旧任务包可直接导入旧格式结果（digest 缺失不再阻断）；也可放弃旧任务重建任务包，新包说明已自带回执】
- **V2.1 外部 AI 导入摘要失配：根因确认与可诊断性修复**——用户导入外部 AI 结果 JSON 报"素材快照摘要不一致"且冻结导入失败，经后端日志定位为两层原因：①粘贴内容实际是任务书 episode-task.json 而非结果 JSON（schema 校验即失败）；②任务包 response_schema 未声明 assets_digest 回执字段且 additionalProperties:false，与导入端硬性要求互相矛盾，任务说明（旧包）也未提示原样复制。已落地：external-ai preview/confirm 失败写入后端错误日志（错误码/消息/冻结标记/结果摘要前缀）；失配面板内联显示双方 assets_digest 前 12 位与失败错误（不再一闪而过 toast）；任务说明第 7 条补"assets_digest 必须原样复制"防错条款；schema 错误文案改为显示实际收到的 schema/version 以便自诊。**遗留（P1，需完整联动）**：response_schema 声明 assets_digest 涉及"发布版 schema + 示例 + 全部结果夹具"的同步更新（有合同测试守卫），已验证直接加字段会波及 15 项测试，须作为独立任务连带发布物一并处理【已于本版本由上一条目清偿】
- **V2.1 AI 配置·高级页宿主化（v1 配置面融入深色工作台）**：`/ai-config/advanced` 此前挂载 v1 时代的 `AiConfig.vue`（自带"本地短剧助手"旧头部与浅色主题，在 V2.1 Rail 壳内形成双头部与刺眼亮色页面）；现新增 V2.1 宿重视图 `AiConfigAdvancedView.vue`——V2.1 页头（返回 AI 配置 / 标题 / "v1 配置面 · 渐进迁移中"徽标）+ 在宿主上把 Element Plus 的 CSS 变量域整体压到 V2.1 深色令牌（card/button/table/dialog/input/tabs/switch/tag/popper 全套），并针对性覆盖 `theme.css` 的 `html.light` 斑马行 `!important` 亮色规则；`AIConfigContent`（2967 行 v1 配置内容）不改一行即融入深色工作台，功能全量保留（Provider CRUD/测试/导入导出/一键配置/SD2 资产管理）。逐元素级重样式按差距清单【简化】项随迁移逐步替换
- **AI 配置页 500 排查结论**：`GET /api/v2/ai-config/overview` 代码路径无缺陷——用户所见 500 为后端进程宕机前的异常表现（进程消失后整页连不上），重启后端后该端点稳定返回 200 全量数据（5 个 Provider/解析顺序/业务映射），已经浏览器实测确认；再次遇到时重启后端并查看启动日志即可定位
- **开发环境：better-sqlite3 升级 v12，后端测试在 Node 24 下恢复可运行（文档与工程）**：backend-node 的 better-sqlite3 由 `^11.6.0` 升级至 `^12.11.1`（v12 起提供 Node 24 ABI 的预编译二进制，prebuilt 直下、无需本地编译链），`engines.node` 由锁死 `22.x` 放宽为 `>=22`。此前依赖按 Node 22（ABI 127）编译、运行环境为 Node 24（ABI 137），`require('better-sqlite3')` 报 NODE_MODULE_VERSION 不匹配，全部后端 SQLite 测试被阻塞；升级后全量后端 1163 测试全绿（含此前被阻塞而无法执行的 v21AssetDetail 与第二梯队 v21AssetRecycleBin / v21ImportSourceLabels / v21StyleVersions 行为测试），前端 385 测试全绿、构建通过。
- **V2.1 诚实性清零：收回三处无实现的「二选一」承诺**：①本集设定音色抽屉的三个预设音色行此前渲染动画波形装饰（暗示可播放音频，实际无任何样音），现撤除假波形并明示「暂无样音」（带真实音频的「已认证音色」行与上传/素材库试听不受影响）；②分镜页「更多」菜单按钮文案「更新分镜结构 / 导入 / 导出」收回「导入」承诺改为「更新分镜结构 / 导出」（菜单与空态本就无导入项），空态副文案同步不再提及「导入分镜结构」；③媒体素材库筛选段移除无数据源的「视频」选项（该页仅聚合项目素材图片，选"视频"恒为空列表），说明文案改为「本页聚合项目素材图片 · 分镜视频请在分镜页查看」，详情抽屉死代码视频分支一并清理。
- **V2.1 高级画布明示「示例投影」口径**：高级画布页头徽标由含混的「只读 · 标准页数据的投影」改为「只读 · 示例投影」，画布内左上角增加说明「图中节点与连线为示例数据，展示关系结构」——该页节点与连线除成片版本号与采用计数外均为硬编码示例，此前未向用户说明。
- **L3 发版门 12 项缺陷修复（QA run_20260912_072912，真实测试验证）**：①V2.1 分镜真实视频生成恢复可用——H3 通道识别正则修复（`minimax_h3_*` 下划线后缀此前恒误判为非 H3 通道，导致 quote 标 h3:false、提交被 409/400 拒绝）；H3 编译、守卫与提交三处服务实例统一为同一带工作流注册表的实例（此前编译与提交的配置快照口径不一致导致草稿恒判过期）；视频提交按镜头引用补传本地参考图路径（真实 H3 工作流要求 1-9 张参考图，含存储根前缀与本站 /static/ URL 还原），修复后 V2.1 制作台可真实派发 ComfyUI H3 视频任务。②V2.1 分镜图生成恢复 ChatGPT 网页通道——生成确认 Sheet 新增通道选择（API / ChatGPT 网页），ChatGPT 通道复用全局生图 store 的环境检查、任务创建、浏览器扩展桥接发送与捕获回填全链，结果自动进入 V2.1 候选列表；通道选择随剧集设置持久化。③AI 配置接口不再返回明文 API Key（列表/详情一律掩码 `********` 并附 has_api_key 标记；把掩码值原样提交不会覆盖已存密钥），前端配置导出不再包含明文 Key（仅 has_api_key），堵住本机密钥经接口/导出文件泄漏的口子。④删除默认 AI 配置时自动向同类型剩余活跃配置移交默认身份（原行为：默认配置被删后四类生成接口 500）。⑤示例项目导入修复：`example_drama` 示例 ZIP 已通过 Git LFS 恢复真实文件（此前为 LFS 指针文本，导入必 500）；导入路由对 LFS 指针文件返回 400 与「执行 git lfs pull」指引而非内部错误，列表接口对不可用示例标记 `available:false`。⑥外部 AI 任务包「单文件任务 JSON」下载修复（此前恒 500）。⑦剧集时长读回不再被分镜聚合无条件覆盖（批量脚本导入写入的显式时长在无分镜时读回为 0 的问题消除）。⑧V2.1 制作包导入补落 `storyboards.source_key`（分镜溯源与人物/场景/道具对齐）。⑨同步故事生成空梗概由 500 改为 400 明确校验提示；分镜变体关联缺 sort_order 等 9 类参数校验错误由 500 改为 400。⑩取消终态异步任务的响应透传 `already_done` 幂等标志。⑪角色主图上传后 `local_path` 落库持久化（此前仅一次性返回，读回恒空）。全量回归：后端 1157 / 前端 371 全绿；ComfyUI H3 真实视频任务派发与 ChatGPT 网页真实生图均经真实环境验证【行为变更：AI 配置接口/导出不再含明文 Key，依赖明文 Key 的外部脚本需改用本机服务端调用】
- **V2.1 P0 第一、二批修复（差距清单四批路线图的第 1-2 批，TDD）**：逐项对应 `docs/vnext/v2.1-implementation-vs-prototype-full-gap-inventory-2026-09-12.md`——①数据安全三处静默失败修复：剧集删除的影响评估与删除接口失败此前零反馈（点击无反应/弹窗停留），现统一 catch 并在弹窗内呈现错误、按钮防重复并换用规范实心危险按钮；编辑项目保存失败静默，现带 Saving 态、弹窗内错误呈现并保持抽屉打开。②剧本 409 静默覆盖修复（P0-14）：「重试保存」此前取最新修订号后直接以本机内容覆盖服务端草稿，多窗口场景静默丢稿；现 409 时拉取服务端草稿全文打开「版本冲突」对比 Modal（本机 vs 服务端双栏），提供「载入最新版本（放弃本机修改）/ 覆盖服务端版本（显式确认）/ 保留现状」三动作，主 CTA 变「解决版本冲突」。③剧集中心普通"制作中"行无打开入口修复（P0-07）：剧集行本体可点击进入该集阶段，主动作按钮统一由 `rowAction` 计算（completed→查看成片、普通制作中→继续制作、空白→开始创建），行内菜单/动作不再误触发行点击。④素材候选与生成记录透出生成元数据（P0-01 后端侧）：`image_generations` 生成写入补落 `model/size`，v21 查询层候选与生成记录透出 `model/size/prompt`（DB 列此前已备但查询不取、生成不写）；前端候选角标显示尺寸（悬停看完整提示词）、生成记录行显示「通道 · 模型 · 尺寸」并支持「查看完整提示词」展开。⑤分镜「生成分镜图」前置确认 Sheet（规格 §8.2 付费动作）：生成前确认对象/通道/本次完整提示词与「只新增候选、不自动采用」结果规则，确认后才执行。已知阻塞：backend-node 的 better-sqlite3 原生模块为 Node 22 ABI 编译、当前 Node 24 环境缺编译链无法 rebuild，本轮新增的后端行为测试（v21AssetDetail）与环境内全部后端 SQLite 测试暂无法执行（前端 371 测试全绿、构建通过、后端改动已过语法检查）；环境修复（安装 VS 编译链或切换 Node 22）后运行 `npm rebuild better-sqlite3 && node --test test/v21AssetDetail.test.js` 即可验证
- **发布前 QA 测试修复（docs/qa/）**：修复六项真实使用中发现的问题——确认剧本后页面误显示空白剧本（现展示已确认内容并可编辑另起草稿、直达设定）；项目素材列表与剧集中心已归档列表因私有状态未声明导致的恒为空；本集设定无法纳入项目人物（现按已确认剧本正文按名并入引用投影，含后端测试）；确认弹窗标题「确认确认剧本」重复；分镜图生成失败（Provider 不可达）无任何提示（现页内警示条可见）。新增非响应式私有状态静态扫描回归测试。全量回归：后端 1156 / 前端 366 / 原型 174 全绿。
- **V2.1 分镜非法 `?shot=` 深链回退与 `current` 空值渲染兜底（T5.2 修复轮 2）**：修复轮 1 的深链守卫使默认选中让位后，非法深链（如书签镜头已被「更新分镜结构」删除）会让分镜页永久卡在骨架屏——`consumeShotQuery` 未命中分支只设提示无兜底选中，`current` 为 null 时模板对 `current.number`/`current.duration` 的无条件求值抛 TypeError 中止渲染。现两处修复：未命中分支回退选中第 1 镜（带 catch，notice「未找到该镜头，已回到第 1 镜」，非法参数经既有 replace 写回为合法值）；内容分支增加 `current` 守卫（未就绪时显示「正在载入镜头…」占位），同时消除 getShot 飞行期间一过性 `current=null` 的渲染抛错窗口。前端 360 测试全绿（含新增合同断言）、build 通过
- **V2.1 分镜 `?shot=` 深链竞态与项目素材 `?asset=` 深链失败保留（T5.2 评审修复）**：分镜页挂载时 `load` 末尾的「默认选中第 1 镜」与深链消费 `consumeShotQuery` 的目标镜头选中并发——两个 `getShot` 竞速，若第 1 镜响应后到，选中与 URL（`writeSceneShotUrl`）会回落第 1 镜，`?shot=` 深链（含成片页「回分镜处理」落点）静默失效且被写回语义固定；现 `load` 内默认选中分支增加 `?shot=` 待消费守卫（深链存在时让位，由 `consumeShotQuery` 单独选中，消除并发），同时深链消费在首次加载失败（尚未 loaded）时让位保留参数，错误态重试经新增 `retryLoad` 在重载成功后补消费，不再在空列表上误判「镜头不存在」并清除深链。项目素材页 `?asset=` 深链同口径：首次加载失败保留参数、重试成功后消费。错误分支口径统一（分镜/成片/项目素材三页）：仅首次加载失败显示错误块（`loadError && !loaded`），已有内容后的重载失败保留内容并以页内提示条/notice 呈现。前端 360 测试全绿（含新增守卫合同断言）、build 通过
...
- **V2.1 实现与交互原型全量差距清单（逐页·逐弹窗·逐子功能）**：新增 `docs/vnext/v2.1-implementation-vs-prototype-full-gap-inventory-2026-09-12.md`——以交互规格与统一原型（含 4869 行原型状态机）为基准，按 7 个域对当前实现做功能级逐项核对：合计仍缺失约 197 项子功能/细节（P0 级 14、P1 级约 89、P2 级约 94），相对 09-11 老审计已修复约 93 项；14 项 P0 含素材候选/生成记录缺提示词与模型尺寸（前后端双层未透出）、素材"高级"标签页空壳、生成确认即伪造完成、剧集中心普通行无入口、项目设置分区整页缺失、制作包资产决策矩阵、删除/保存静默失败、迁移恢复闭环断裂、AI 配置 20 类错误场景机、ChatGPT 环境检测整链、剧本 409 静默覆盖等，每项附原型依据与 file:line 证据，并给出四批修复路线图。同日经产品视角复核追加「产品处置结论」：以单人短剧制作台为准把全部缺失标记为【修复/简化/裁剪/二选一】四种处置——裁剪约 30 项（画布交互化、主题切换、通知汇总等单人低价值项）除名，实际待修约 165 项；P0-04/P0-05 降为 P1，浏览器恢复副本与制作头运行任务死值上调，6 项"承诺未做"标记为须最先清零的二选一（音色试听、分镜导入菜单、媒体库视频聚合、来源标签区分、画布示例标注、风格版本占位）
