...
- **V2.1 归档导入真实校验（`POST /api/v2/archive/validate`，Task 5-C）**：导入项目归档页的「校验并确认」从三条静态检查占位升级为真实校验。后端新端点收 JSON `{ path }`（本地 zip 路径，不引入上传基建；导入动作本身保持既有 `POST /api/v1/dramas/import` 文件上传），用 adm-zip 读取归档并返回七项检查矩阵——格式 / 协议版本 / 结构 / 解压完整 / 媒体条目 / 项目重名 / 磁盘空间，逐项「通过 / 警告 / 阻断 / 版本不支持」并附原因，另附概要指标（项目名 / 剧集数 / 媒体文件数 / 条目数 / 预计解压大小 / zip 大小）；硬错误按口径返回：路径缺失 400、文件不存在 404、非有效 zip 400。版本校验对照 2.1 协议：现有 V1 导出（project.json version=1.7）如实返回 `unsupported` 与实际版本号，不伪装通过。页面侧（归档导入页）：新增归档本地路径输入（校验失败保留输入可就地重试），三条静态检查行替换为后端检查矩阵渲染，概要区显示指标；unsupported 时呈现"检测到归档版本 vX，当前仅支持 2.1"横幅与「重新选择」；页面注明「导入不覆盖现有项目」，仅填路径未选文件时导入按钮禁用并说明导入走文件上传。后端配套 `backend-node/src/v21/import/archiveValidateService.js`，新增 5 条测试（400/404/坏 zip/真实导出全矩阵与重名 warn/缺 manifest 结构阻断）
