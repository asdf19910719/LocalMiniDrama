...
- **V2.1 更换画面风格对齐旧版选择器交互（预览图 / 一步更换 / 详情 / 创建我的风格）**：项目概览「更换风格」弹窗四项修复——①风格卡片由渐变占位块改为渲染真实预览图（169 张风格预览图本就在 `frontweb/public/style-thumbs/` 且接口已返回，此前未接线），无图回退占位渐变；②交互由「选择 → 确认影响 → 应用」两步简化为「点卡片选中 → 右下角确认更换」一步（影响口径保留在选择页底部说明），选中卡片加 2px 主题色描边与「已选」徽标——此前选中态仅细边框几乎不可见，被误认为点击无反应；③卡片新增「详情」视图：大图预览、名称/说明/分类与中英文风格提示词展开查看（对齐旧版风格选择器的详情抽屉）；④恢复「创建我的风格」入口（中英文名称/说明/中英文提示词五字段必填，走既有 `POST /api/v1/styles/custom` 端点落 `custom_styles` 表，创建后自动切到「我的风格」页签并选中），并删除「需安装自定义风格目录后开放」的错误禁用文案——该创建能力后端一直默认支持，无需任何目录安装。
