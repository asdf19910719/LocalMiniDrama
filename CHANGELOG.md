...
- **V2.1 外部 AI 结果「按冻结快照导入」通道与校验失败可见（P0-3）**：外部 AI 制作向导的结果步在校验未通过时不再只有预览步能看到明细——就地渲染逐项检查清单（每项标签 + 通过标记 + 详情，未通过项标红）并提示"校验未通过，请修正结果 JSON 或返回上一步"；素材快照摘要失配（ASSETS_DIGEST_MISMATCH）时弹出专用三选面板：按冻结快照导入（预览/确认导入请求支持可选 `frozenSnapshot`，仅跳过摘要失配单项校验、其余校验照常，成功后返回与 `episode_imports` 审计记录均标注 `frozenSnapshot: true`）、放弃并创建新任务（取消任务后回第一步）、返回修改结果 JSON；前端 `v21.previewImport/confirmImport` 封装支持透传该选项
...
- **V2.1 外部 AI 向导「可离页恢复」真正生效（P0-3）**：外部 AI 制作向导此前文案宣称"任务已持久化，可离页"，但刷新后状态全丢、也没有任何找回入口。现创建任务包成功后向导 URL 写入 `?taskId=`，刷新或离页后回到向导自动恢复——按后端返回的当前步直接落到「等待外部结果」或「已导入草稿」（展示目标集号等真实任务信息）；任务不存在或已取消时明确提示"任务不存在或已取消"并停留第一步；取消任务后清理 URL 查询参数。剧集中心新增「外部 AI 任务」卡片区（剧集列表上方，无任务时不显示），按创建时间倒序列出本项目全部外部任务（等待外部结果 / 已导入 / 已取消状态标签 + 目标集号 + 包 ID 前 8 位 + 创建时间），等待中任务可「打开向导」（直达 `external-ai?taskId=`）或就地「取消」，已导入任务可「打开剧本」直达该集剧本页（后端配套新增项目外部任务列表端点 `GET /api/v2/projects/:id/external-ai/tasks`）
