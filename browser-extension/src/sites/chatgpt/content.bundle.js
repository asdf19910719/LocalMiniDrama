(() => {
  ...
  var ChatGPTAdapter = class {
    ...
    composer() {
      return visibleControl(this.document, selectors.composer);
    }
    sendButton() {
    ...
    beginAttempt(identity, onResult, onError = () => {
      ...
      const discover = () => {
        const candidates = [...root.querySelectorAll?.(selectors.assistant) || []].filter((node) => !preExisting.has(node) && !isUserTurn(node)).map((node) => ({ node, id: messageIdentity(node)?.messageId })).filter((entry) => entry.id && !isTransientAssistantIdentity(entry.id) && (!known.has(entry.id) || entry.id === activeAssistantId && entry.node !== activeStop?.root));
