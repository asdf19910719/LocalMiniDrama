(() => {
  ...
  var ChatGPTAdapter = class {
    ...
    beginAttempt(identity, onResult, onError = () => {
      ...
      const preExistingTurnIds = new Set(existingTurns.map((node) => messageIdentity(node)?.messageId).filter(Boolean));
      ...
      const discover = () => {
        if (!activeUserId) {
          const newUser = [...root.querySelectorAll?.(selectors.turn) || []].filter((node) => !preExistingTurns.has(node) && isUserTurn(node) && !preExistingTurnIds.has(messageIdentity(node)?.messageId)).map((node) => messageIdentity(node)?.messageId).find((messageId) => messageId && !isTransientAssistantIdentity(messageId));
          if (newUser) {
