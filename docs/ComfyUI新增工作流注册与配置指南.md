...
先把文件复制到项目受控路径，再在 `backend-node` 目录对最终文件执行只读分析。脚本与注册表使用同一套换行归一化 SHA 规则，因此 Windows CRLF 与 Git 检出为 LF 时不会产生伪漂移：
...
- 工作流文本统一换行为 LF 后的 `workflowSha256`（不会解析再序列化 JSON）；
