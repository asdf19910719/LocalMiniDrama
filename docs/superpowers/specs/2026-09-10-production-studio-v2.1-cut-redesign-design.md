...
**优先复用（当前产品已实现）**：
- `video_merges` + videoMergeService 整集合片（含保留 H3/原声 + TTS/BGM 混音——本页"成片设置"的混音后端）。
- 超分管线（video_upscale_jobs）、SRT 导出、分镜页候选事实源（director 候选表 + storyboards 采用状态）。
- 统一任务模型（async_tasks：进度/取消/失败重试）驱动"生成成片"任务。

**需新开发**：成片版本表（来源镜头候选快照 + 合成设置 + 产物路径 + 历史版本）、完成度门禁 API（x/y 已完成、需确认旧镜头统计）、短片页单屏前端。

**需改造**：`video_merges` 一次性合成升级为版本化合成任务；后期链（规格/风格/超分/字幕/混音/编码）内部顺序执行，不暴露 UI。
