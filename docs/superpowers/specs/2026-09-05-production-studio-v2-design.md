...
2026-09-09 已确认的[个人创作者体验收敛与 ChatGPT Web 提级设计](./2026-09-09-production-studio-v2.1-personal-creator-simplification-design.md)是本文的产品呈现细化。它不改变 Revision、Candidate、任务恢复、精确引用、Picture Lock、Post/Delivery 或 Phase 6 一次切换等内部模型，但取代本文历史段落中的旧用户界面称呼和入口层级：全局导航为“项目、资产库、任务、设置”；项目内为“概览、剧集、项目素材、项目设置”；单集阶段为“剧本、本集素材、分镜、成片”。自由创作进入“更多工具”，高级画布只从阶段内高级入口进入。用户点击“开始合片”时由系统内部创建 Picture Lock，默认界面不要求用户理解该术语。

ChatGPT 网页自动化是 Product Core 一级可选生图通道，可设为全局或项目默认。解析顺序固定为“本次选择→项目默认→全局默认→安装默认”；环境检测零生成费用，环境不可用时保留已保存默认值，只允许用户修复环境或对当前任务显式改用 API/ComfyUI。所有网页结果先进入候选，不自动替换当前使用项。
...
不同生产对象不强行共用一套操作层级。项目素材图片/音色只有“候选”和“当前使用项”：生成/上传只建候选，用户点击候选即写 current pointer，旧 current 留在历史；不再暴露临时选中、采用和对象批准。本集素材阶段只在必需对象通过自动检查后保存不可变 episode asset set snapshot；用户唯一主操作为“进入分镜”，不再增加阶段确认或审批。视频候选采用、剧本/Look/Shot 确认、Picture Lock 与交付仍保留各自必要的决策。`waived` 是带原因和过期规则的独立 Gate exception，不是对象或阶段状态。
...
- 页面返回入口为“返回剧集”，主标题为“剧本”，项目/剧集作为面包屑；内部 revision id 只在技术详情存在。
- 草稿持久化只投影为“有未保存修改 → 保存中 → 已保存于具体时间”。保存失败、离线、409、空剧本和确认处理中禁用确认；浏览器恢复副本只在与服务端内容不同时出现。
- AI 续写和改写先创建候选及差异，用户应用后才写草稿；生成多集和项目复制是次级入口。历史版本只读，恢复只派生新草稿。
- 草稿保存后后台轻量计算预计素材变化；确认检查集中展示场次、字数、预计时长、素材决定和具体下游数量。确认成功后主操作转为“进入本集素材”。
- 场次提供结构化内外景、地点、时间和正文；单场次重命名、排序、复制、删除撤销属于 P1，不建设专业编剧格式、多选批量编辑或团队审批。
- 后续阶段可以浏览旧快照，但未确认草稿不能成为收费生成输入。确认永不自动删除、覆盖或重新生成已有媒体。
...
项目级聚合入口的用户可见名称固定为“项目设置”（内部原型路由可继续沿用 `project-bible` 以保持深链可恢复）。首屏按“项目资料与项目画面风格 → 生产对象概览 → 外部 AI 创作”组织，页面 Header 不设置统一保存或外部 AI 主按钮。项目资料独立维护名称、封面、画幅、题材、默认单集时长和输出偏好，本地位置只读；外部 AI 创作上下文独立维护故事基础设定、不可改变的设定和跨集连续性备注。人物/状态、场景和道具只读聚合并精确跳转项目素材处理，不能在本页创建或采用媒体候选。Project Look 保持项目作用域，基础选择、影响预览和创建新版本形成完整流程，不绑定任意剧集。
...
V2.1 首版不在“项目设置”中建设百科式结构化世界观事实库、伏笔追踪器、人物关系图或事件时间线，也不自动推断剧情事实。“外部 AI 创作”卡把“编辑创作上下文”“复制创作上下文”和“创建外部 AI 制作任务”分成独立动作：复制只预览、复制或下载 Markdown；制作任务复用项目剧集中心的统一目标选择，按“选择目标集号 → 当前数据预览 → 创建任务包 → 外部创作 → V2.1 JSON 回流 → 五步导入确认”执行。生成或复制上下文本身不调用外部模型、不创建剧集，也不产生费用；外部新增人物/状态/场景/道具必须经过导入预览确认，不能覆盖已有 Look、素材当前使用项或自动启动媒体任务。

### 7.5 资产详情抽屉

项目素材采用“生产对象优先”，只管理当前项目可跨集复用的人物/状态（含人物音色）、场景和道具及其当前图、媒体候选、资料修订、生成记录和使用位置。分镜图片与逐镜正式视频候选归分镜阶段；整集审核、环境声、音效、音乐与时间线归成片阶段；全局个人资产库负责跨项目复用；本集素材负责选择本集实际使用项并保存不可变快照。

首屏统计只保留人物、场景、道具和需要处理；默认显示类型、搜索、简化状态和排序，使用/来源/媒体/剧集筛选及紧凑列表切换收入“更多筛选”。批量操作先进入多选并由用户明确选择范围。人物状态数显示在人物卡，音色进入人物详情，不建立平级声音卡。所有素材详情固定为“概览 / 版本与候选 / 使用位置 / 生成记录 / 高级”，默认只展开当前状态和主视图，统一显示：
...
从个人资产库进入项目时必须选择“引用固定版本”或“复制到项目”；两者均不自动随库更新漂移。资料修订、媒体候选和当前图指针独立；生成和上传只创建候选，点击候选即设为当前图并可撤销，旧图保留在历史。变化只影响未来选择，已确认剧集快照不自动变化。场景与道具使用“稳定对象 → 剧情状态 → 视图参考 → 媒体候选”关系；人物形象与人物音色分别维护候选和 current pointer。API、ChatGPT 网页和 ComfyUI 共用 Generation Envelope、能力检查和可恢复异步任务。用户进入分镜时系统保存精确版本/hash 快照；音色只有在本集音频策略明确依赖且缺失时才阻断。从音视频提取音色前必须确认来源和许可。批量操作只作用于明确所选项，支持部分成功且不自动设为当前图。
...
项目归档导入保持独立 Page，并固定为“选择归档 → 校验并确认 → 导入结果”三阶段。它只接受 `local-mini-drama.project-archive@2.1` 并创建新项目，不承担已有项目恢复、单集包或分镜文件导入。校验必须分别给出版本、结构、Hash、媒体、项目名称和含临时解压的峰值空间状态；缺失媒体为允许继续的警告，保留引用和 Hash。提交后创建可从全局任务中心恢复的持久化本地任务；失败完整回滚数据库、媒体和临时文件，成功后历史任务仅恢复为只读记录，绝不自动重新执行或调用 Provider。
...
```text
顶部：阶段导航、集名、当前镜头与整集完成度
工具栏：场次、当前镜头、完成度、批量生成、更多
左：镜头描述 / 引用素材 / 时间段 / 连续性，默认只展开当前区
中：分镜图 / 视频主预览、候选胶片条、采用 / 撤销 / 重试
右：镜头设计 / 生成 / 历史；H3、Manifest 与技术参数进入高级设置
底：固定镜头轨，显示当前、未完成、生成中、失败、过期与已采用
```

页面围绕单一逐镜链路组织：选择镜头 → 检查描述与引用 → 生成/导入分镜图 → 设为当前 → 生成视频候选 → 主播放器审核 → 采用 → 下一镜。三栏用于减少上下文跳转，不允许把全部底层字段同时展开。
...
右侧默认生成标签只显示可读生成描述、模型、比例、清晰度、计划/请求时长、费用、预计等待和生成动作。目标 Video Recipe、Reference Manifest、H3/Omni 结构、技术 ID、能力限制与完整 Prompt 进入高级设置。中栏必须提供图片/视频双模式主预览；视频候选成功载入播放器后才可采用，不强制完整播放。任何采用与撤销都同步候选卡、底部镜头轨和成片准备度。

改选当前分镜图后，已编译 Prompt/H3 与未提交预检必须失效；完成的旧视频保留并标记“基于旧分镜图”，原采用结果暂时退出完成计数，待用户明确继续使用或改选。视频提交统一检查必需引用、Provider 能力、Prompt/H3 有效性和同镜活动任务；任一失败或已有运行任务均阻断新付费提交。

结构更新入口严格区分来源和写入边界：

- `更新分镜结构` 只读取当前已确认 Script revision。运行前展示来源 revision、结构模型、预计费用/耗时；运行后展示新增、变化、拟删除、不变镜头 diff，人工锁定冲突逐项处理，最终只创建新 Shot revision；
