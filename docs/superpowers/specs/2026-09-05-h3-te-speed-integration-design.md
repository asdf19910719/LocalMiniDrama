...
集成采用独立工作流变体，不覆盖官方 + Sage 基线，不在同一任务中静默回退算法。完成本机验收后，项目交互调整为 TE-Speed 默认开启、官方 + Sage 可一键关闭加速回退。
...
- 保留当前 `minimax_h3_director_r2v` 的图结构、hash、回滚能力和历史任务语义；新建面板默认选择 TE-Speed。
...
验收已通过，随后按用户决定将 TE-Speed 改为默认开启。项目面板开关开启时使用 `minimax_h3_director_r2v_te_speed`，关闭时使用 `minimax_h3_director_r2v`；现有任务仍按不可变快照恢复，不自动换算法。

## 11. 最终开关与提示词策略

- LocalMiniDrama 前端只允许在官方与 TE 两个已注册 ID 之间切换，默认开启 TE-Speed；后端对工作流选择和草稿工作流 ID 做同样的精确校验。
- 每个工作流分别保存 H3 草稿。切换时先保存当前草稿，再读取目标工作流草稿，防止用官方草稿提交 TE 任务或反向混用。
- `01-官方-H3-R2V.json` 保持单文件，节点链为 `UNET -> Sage -> TE-Speed -> Director`。TE 节点默认执行，选中后按 `Ctrl+B` 旁路即关闭。
- 后续 H3 A/B 测试统一附加高快门清晰动作约束，并在提交 GPU 前拒绝 `natural/cinematic/... motion blur` 等正向运动模糊要求。01 的全局提示词、分段提示词和负面词也已同步清理。
