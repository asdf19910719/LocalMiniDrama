...
- **多候选适用于全部目标类型**:候选数量取决于 ChatGPT 单次生成返回几张图,与目标类型无关;角色/场景/道具/分镜四类卡片使用一致的候选条与切换交互。
- **卡片图片更新时间**:卡片图片区下方常驻小字"图更新于 MM-DD HH:mm";自动定稿、手动换图、批量恢复后同步刷新。取值为绑定主图时写入目标行的 `image_updated_at`(该候选的导入时间),不使用卡片行自身的 `updated_at`(编辑名称等操作也会改它,语义不准)。
...
- `characters`/`scenes`/`props`/`storyboards` 四表通过 `ensureColumns()` 自动迁移新增 `image_updated_at`(TEXT,ISO 时间,可空)。
- 所有绑定入口(`bindAsset`、`bindResult` 的分镜分支、批量恢复)统一在绑定时写入 `image_updated_at` = 该候选导入时间;换主图即刷新。
...
- 卡片:角色/场景/道具/分镜卡片在图片区下方渲染"图更新于 …"小字行,数据来自目标的 `image_updated_at`;无图时显示占位文案或隐藏。
...
| 用户换主图 | `select-result` 现有逻辑,历史图进 extra_images,`image_updated_at` 刷新 |

## 测试策略(TDD)

- 后端:导入服务决策测试(首张自动定稿/多张不抢占/开关关闭/目标缺失/竞态转态)、批量恢复路由测试(混合状态批次)、设置项读写、绑定写 `image_updated_at`。
- 前端:store 通知分支、抽屉 completed 候选渲染源码测试、批量恢复入口显隐、卡片更新时间渲染。
- 回归:后端/前端/扩展全量 + 生产构建;真实链路以一次 3-5 任务连续生成做冒烟(角色/场景/道具各一)。

## 验收标准

1. 连续生成角色/场景/道具各 1 张,全部在不打开抽屉的情况下出现在项目页对应位置。
2. 对已自动定稿的任务换主图后,旧图进入历史、新主图即时生效,卡片"图更新于"时间刷新。
3. 多候选任务(任一目标类型)的候选条可切换主图,切换可逆。
4. 关闭开关后行为与现状一致。
5. 09-01 存量 3 张待选候选可通过"全部采用首选"一键恢复。
