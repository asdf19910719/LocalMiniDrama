...
- [x] **Step 1: Add failing navigation and terminology tests**
...
- [x] **Step 2: Add failing source and episode-material tests**
...
Also assert every source card resolves to its own next interaction after the project shell is created. Add an import-conflict contract in which the unresolved row exposes explicit reuse/create choices, exposes skip only when the source reference is optional, and cannot advance until a valid choice is committed.

- [x] **Step 3: Add failing ChatGPT default and environment tests**
...
- [x] **Step 4: Add failing task-center and Cut-language tests**
...
Assert the `exported` Cut scenario has a completed base composite, a persisted terminal upscale decision, passed encode validation and no delivery blocker; it must not display `optional` upscale or `waiting` encoding while export is already complete.
...
function getEpisodeCreationSourceGroups() {
  return {
    primary: [
      { id:'script-import', label:'我有剧本' },
      { id:'ai-script', label:'让 AI 帮我写' },
      { id:'package-import', label:'我有制作包', secondaryAction:'start-external-ai-collaboration' },
    ],
    more: ['blank', 'novel-split', 'source-video'],
  };
}
```

- [x] **Step 1: Register the task page**
...
- [x] **Step 2: Add product and stage navigation models**
...
- [x] **Step 3: Group the six source capabilities**
...
On the new-project page, source cards only select a start mode. The contextual primary action validates and creates the project exactly once, then routes script import, package import, blank creation, AI script, novel split and source video to their respective first-episode drawers/pages. The new-project context may only create episode 1 and must not expose existing-project targets. Do not expose Project Shell terminology or collapse sources into a shared success placeholder.

- [x] **Step 4: Make asset conflict resolution explicit**

Represent every unresolved import match with available decisions and a selected decision. Reuse requires an existing target, create produces an independent project material, and skip is available only for optional/unreferenced source objects. Committing a decision updates the preview; merely opening the chooser does not mutate it.
...
- [x] **Step 1: Implement deterministic channel resolution**
...
- [x] **Step 2: Model environment checks**
...
- [x] **Step 3: Update AI configuration UI**
...
- [x] **Step 4: Update asset/storyboard generation controls**
...
- [x] **Step 1: Build task-center grouping and filters**
...
- [x] **Step 2: Render the global task page**
...
- [x] **Step 3: Connect project task summaries**
...
- [x] **Step 1: Move external AI actions into a collaboration card**
...
- [x] **Step 2: Make Episode Materials delta-first**
...
- [x] **Step 3: Replace internal terminology in default surfaces**
...
- [x] **Step 1: Rename the primary operation**
...
- [x] **Step 2: Add confirmation and success landing**
...
Make terminal scenarios internally consistent. In particular, `exported` must report encode validation as passed and upscale as run/skipped/fallback according to its persisted decision; no completed delivery may retain `waiting` or undecided post steps.
...
- [x] **Step 1: Record replacement decisions**
...
- [x] **Step 2: Update architecture and UI contracts**
...
- [x] **Step 3: Update migration and E2E mappings**
...
- [x] **Step 4: Correct the design review conclusion**
...
- [x] **Step 2: Parse the inline browser script and count contracts**
...
- [x] **Step 3: Browser-inspect every default route and high-risk scenario**
...
- [x] **Step 4: Check documentation integrity**
