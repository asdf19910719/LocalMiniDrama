...
修复为分层条件等待：`beginAttempt` 前等待一次，绑定观察器后、调用 `submit` 前再次等待一次，内容脚本收到最终提交请求后再对实时按钮状态做一次原子前置等待。每层等待最长 120 秒；极端情况下提交前最多累计 360 秒，这段时间只用于 ChatGPT 页面就绪，不包含图片生成耗时。对当前真实登录环境而言，120 秒足以覆盖页面状态切换：新扩展重载后本轮 9 连点从首项排队到末项候选回传均正常，单项生成与切换未触发超时。
