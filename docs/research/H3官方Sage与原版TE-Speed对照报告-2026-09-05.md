...
TE-Speed 作为第二个独立工作流接入，官方 Sage 工作流仍是明确的回滚路径。TE 工作流在
...
- 默认开启的 TE 工作流：`minimax_h3_director_r2v_te_speed`
- 关闭加速后的官方工作流：`minimax_h3_director_r2v`
- ComfyUI 编号工作流：`12-官方-H3-R2V-Sage-TE-Speed实验.json`
- 项目视频面板的 `TE-Speed 加速` 开关默认开启，只在上述两个固定 ID 之间选择；现有任务的不可变快照不会被静默切换。
- 关闭开关会创建新的官方工作流任务，不会把已开始的 TE 任务静默改跑官方算法。
- `01-官方-H3-R2V.json` 已直接加入原作者节点并默认开启；选中该节点按 `Ctrl+B` 可旁路关闭。
- 如果插件二进制、注册入口、ComfyUI 或工作流哈希变化，应重新降级为 `configured` 并复测。

## 动作清晰度测试策略更新

早期快动作样本用于测量既有提示词下的性能，历史结果不回写。后续测试统一使用高快门、逐帧清晰约束：脸、眼睛、头发、衣物、身体轮廓和运动物体在每帧保持清晰，并明确禁止 motion blur、temporal smearing、ghosting 与 double edges。对照脚本在提交 GPU 前拒绝任何正向运动模糊要求，避免把提示词主动要求的模糊误判为 TE-Speed 画质退化。
