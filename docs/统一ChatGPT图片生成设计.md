...
> 主分支同步（2026-08-28）：统一 ChatGPT 图片生成已合并到本地 `main`（`a18c1a9`），并补充 H3 默认视频请求字段测试（`1a7a2f8`）。合并后 Node 22.22.3 后端全量测试 `300/300`、浏览器扩展 `39/39`、前端测试 `57/57`、前端生产构建均通过；`origin/main` 已确认是本地 `main` 的祖先，本地领先 113 个提交。工作流注册表文件保持已登记的 SHA-256。
