...
const registry = {
  workflows: [
    { id: 'verified', label: 'Verified workflow', status: 'verified', family: 'video', variant: 'v1', adapter: 'a', acceleration: { kind: 'test', approximate: true }, execution: { promptContract: 'free_text_v1' }, capabilities: { modes: ['text'] } },
...
describe('workflow catalog', () => {
  it('keeps every governance state and exposes execution metadata', () => {
    ...
    assert.equal(items[0].label, 'Verified workflow');
    assert.deepEqual(items[0].acceleration, { kind: 'test', approximate: true });
  });
