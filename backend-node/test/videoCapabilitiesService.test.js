...
describe('video capabilities service', () => {
  it('loads the API workflow graph before asking the adapter for capabilities', () => {
    ...
    assert.equal(result.workflow.id, 'minimax_h3_director_r2v');
    assert.equal(result.workflow.label, '官方多参考图（Sage）');
    assert.equal(result.capabilities.supportsTESpeed, false);
    assert.equal(result.capabilities.approximateAcceleration, false);
    db.close();
  });

  it('exposes the TE-Speed label and approximate acceleration capability', () => {
    const db = new Database(':memory:');
    db.exec(`CREATE TABLE ai_service_configs (
      id INTEGER PRIMARY KEY AUTOINCREMENT, service_type TEXT, provider TEXT,
      api_protocol TEXT, base_url TEXT, api_key TEXT, model TEXT, default_model TEXT,
    ...
    db.prepare(`INSERT INTO ai_service_configs
      (service_type, provider, api_protocol, base_url, api_key, model, default_model, settings, is_default, is_active)
      VALUES ('video', 'comfyui', 'comfyui', 'http://127.0.0.1:8188', '', ?, ?, '{}', 1, 1)`)
      .run(JSON.stringify(['minimax_h3_director_r2v_te_speed']), 'minimax_h3_director_r2v_te_speed');
    const registry = loadRegistry(path.resolve(__dirname, '..', 'configs', 'director-workflows.json'));
    const configured = registry.workflows.find((workflow) => workflow.id === 'minimax_h3_director_r2v_te_speed');
    configured.status = 'verified';
    configured.verifiedEvidence = 'test-fixture';
    const service = createUnifiedVideoGenerationService({
      db,
      log: { info() {}, warn() {}, error() {} },
      providerRegistry: { has() { return false; } },
    ...
    const result = service.getVideoCapabilities();
    assert.equal(result.workflow.label, '官方多参考图（Sage + TE-Speed 实验）');
    assert.equal(result.capabilities.supportsTESpeed, true);
    assert.equal(result.capabilities.approximateAcceleration, true);
    db.close();
  });
});
