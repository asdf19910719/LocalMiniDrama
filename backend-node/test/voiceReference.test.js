...
describe('H3 voice reference (optional)', () => {
  ...
  beforeEach(() => {
    ...
    db.exec(`
      CREATE TABLE characters (
        ...
        seedance2_voice_local_path TEXT, seedance2_voice_asset TEXT
      );
  ...
  it('resolves only active voice assets of the storyboard bound characters', () => {
    const insertChar = db.prepare(`INSERT INTO characters (id, drama_id, name, deleted_at, seedance2_voice_local_path, seedance2_voice_asset)
      VALUES (?, 3, ?, NULL, ?, ?)`);
    insertChar.run(1, '云青', 'storage/voice/yunqing.wav', JSON.stringify({ status: 'active' }));
    insertChar.run(2, '老郎中', 'storage/voice/stale.wav', JSON.stringify({ status: 'stale' }));
    insertChar.run(3, '无音色', null, null);
    db.prepare('INSERT INTO storyboard_characters (storyboard_id, character_id) VALUES (501, 1), (501, 2), (501, 3)').run();

    const audios = resolveVoiceReferenceAudios(db, 501);
    assert.equal(audios.length, 1);
    assert.equal(audios[0].characterName, '云青');
    assert.equal(audios[0].audioFile, 'storage/voice/yunqing.wav');
  });
  ...
  it('fills timeline refAudios from reference audios without changing task type', () => {
    ...
    const output = buildStructuredWorkflowPrompt(workflow, {
      prompt: '测试提示词',
      durationSeconds: 5,
      referenceAudios: [{ audioFile: 'storage/voice/yunqing.wav', characterName: '云青' }],
    });
    ...
    assert.equal(timeline.global.refAudios[0].audioFile, 'storage/voice/yunqing.wav');
