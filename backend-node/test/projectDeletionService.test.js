...
test('permanent episode deletion removes only episode descendants and preserves project assets and sibling episodes', (t) => {
  ...
  insert("INSERT INTO image_generations (id, drama_id, task_id, status, created_at, updated_at) VALUES (118, 1, 'image-task-prop-target', 'completed', ?, ?), (119, 1, 'image-task-prop-sibling', 'completed', ?, ?), (120, 1, 'image-task-project-library', 'completed', ?, ?)", now, now, now, now, now, now);
  insert("INSERT INTO image_generation_batches (id, drama_id, resource_scope, generation_channel, created_at, updated_at) VALUES ('batch-episode-only', 1, 'episode', 'external', ?, ?), ('batch-shared', 1, 'project', 'external', ?, ?)", now, now, now, now);
  insert("INSERT INTO image_generation_tasks (id, drama_id, target_type, target_id, generation_channel, batch_id, image_generation_id, external_job_id, created_at, updated_at) VALUES ('task-prop-target', 1, 'prop', 14, 'external', 'batch-episode-only', 118, 'job-prop-target', ?, ?), ('task-prop-sibling', 1, 'prop', 114, 'external', 'batch-shared', 119, 'job-prop-sibling', ?, ?), ('task-project-library', 1, 'character', 12, 'external', 'batch-shared', 120, 'job-project-library', ?, ?)", now, now, now, now, now, now);
  insert("INSERT INTO external_generation_jobs (id, drama_id, image_generation_task_id, site, prompt_snapshot, prompt_hash, created_at, updated_at) VALUES ('job-prop-target', 1, 'task-prop-target', 'site', '目标道具', 'hash-prop-target', ?, ?), ('job-prop-sibling', 1, 'task-prop-sibling', 'site', '保留道具', 'hash-prop-sibling', ?, ?), ('job-project-library', 1, 'task-project-library', 'site', '项目角色', 'hash-project-library', ?, ?)", now, now, now, now, now, now);
  insert("INSERT INTO external_generation_attempts (id, job_id, sequence, created_at, updated_at) VALUES ('attempt-prop-target', 'job-prop-target', 1, ?, ?), ('attempt-prop-sibling', 'job-prop-sibling', 1, ?, ?), ('attempt-project-library', 'job-project-library', 1, ?, ?)", now, now, now, now, now, now);
  insert("INSERT INTO external_generation_results (id, attempt_id, result_index, created_at, updated_at) VALUES ('result-prop-target', 'attempt-prop-target', 1, ?, ?), ('result-prop-sibling', 'attempt-prop-sibling', 1, ?, ?), ('result-project-library', 'attempt-project-library', 1, ?, ?)", now, now, now, now, now, now);
  insert("INSERT INTO external_generation_events (id, attempt_id, idempotency_key, sequence, event_type, created_at) VALUES ('event-prop-target', 'attempt-prop-target', 'idem-prop-target', 1, 'done', ?), ('event-prop-sibling', 'attempt-prop-sibling', 'idem-prop-sibling', 1, 'done', ?), ('event-project-library', 'attempt-project-library', 'idem-project-library', 1, 'done', ?)", now, now, now);
  insert("INSERT INTO director_candidates (id, group_id, artifact_id, job_id, created_at, updated_at) VALUES ('candidate-shared', 'group-keep', 'artifact-target', 'director-job-target', ?, ?)", now, now);
  ...
  assert.equal(count(db, 'image_generations', 'id = ?', 118), 0);
  assert.equal(count(db, 'video_generations', 'id = ?', 17), 0);
  assert.equal(count(db, 'video_merges', 'id = ?', 18), 0);
  assert.equal(count(db, 'image_generation_tasks', 'id = ?', 'task-prop-target'), 0);
  assert.equal(count(db, 'image_generation_batches', 'id = ?', 'batch-episode-only'), 0);
  assert.equal(count(db, 'external_generation_jobs', 'id = ?', 'job-prop-target'), 0);
  assert.equal(count(db, 'external_generation_attempts', 'id = ?', 'attempt-prop-target'), 0);
  assert.equal(count(db, 'external_generation_results', 'id = ?', 'result-prop-target'), 0);
  assert.equal(count(db, 'external_generation_events', 'id = ?', 'event-prop-target'), 0);
  ...
  assert.equal(count(db, 'image_generations', 'id = ?', 119), 1);
  assert.equal(count(db, 'image_generations', 'id = ?', 120), 1);
  ...
  assert.equal(count(db, 'image_generation_tasks', 'id = ?', 'task-prop-sibling'), 1);
  assert.equal(count(db, 'image_generation_tasks', 'id = ?', 'task-project-library'), 1);
  assert.equal(count(db, 'image_generation_batches', 'id = ?', 'batch-shared'), 1);
  assert.equal(count(db, 'external_generation_jobs', 'id = ?', 'job-prop-sibling'), 1);
  assert.equal(count(db, 'external_generation_jobs', 'id = ?', 'job-project-library'), 1);
  assert.equal(count(db, 'external_generation_attempts', 'id = ?', 'attempt-prop-sibling'), 1);
  assert.equal(count(db, 'external_generation_results', 'id = ?', 'result-prop-sibling'), 1);
  assert.equal(count(db, 'external_generation_events', 'id = ?', 'event-prop-sibling'), 1);
  assert.equal(count(db, 'director_candidates', 'id = ?', 'candidate-shared'), 1);
  assert.equal(count(db, 'director_artifacts', 'id = ?', 'artifact-target'), 1);
  assert.equal(count(db, 'director_jobs', 'id = ?', 'director-job-target'), 1);
  assert.equal(count(db, 'director_anchors', 'id = ?', 'anchor-target-source'), 1);
