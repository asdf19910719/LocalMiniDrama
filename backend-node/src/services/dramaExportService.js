  ...
  const zipData = {
    ...
    characters: characters.map((c, idx) => {
      ...
      const variants = (variantsByCharId[c.id] || []).map((v) => {
        ...
        return {
          ...
          asset_mode: v.asset_mode || 'SINGLE',
          use_identity_reference: v.use_identity_reference === 0 ? 0 : 1,
      ...
      return {
        ...
        negative_prompt: c.negative_prompt || null,
        asset_mode: c.asset_mode || 'TURNAROUND',
    ...
    scenes: dedupedScenes.map(s => {
      ...
      return {
        ...
        asset_mode: s.asset_mode || 'NORMAL',
