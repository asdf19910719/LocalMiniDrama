...
function resolveRequestedWorkflow({ input = {}, resolved, registry, allowExperimental = false } = {}) {
  ...
  const selectedWorkflowId = unique[0]
    ...
    || text(resolved.model)
    || allowed[0];
  const configuredWorkflowId = text(resolved.config.default_model) || text(resolved.model) || allowed[0];
  if (!selectedWorkflowId || (!allowed.includes(selectedWorkflowId)
    && !isSwitchableH3WorkflowPair(configuredWorkflowId, selectedWorkflowId))) {
